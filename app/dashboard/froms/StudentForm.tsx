...
const StudnetForm = ({ type, data }: { type: 'create' | 'update' | 'delete'; data?: any }) => {
  const {
    register,
    handleSubmit,
    formState: { errors }
  ...
  return (
    <form className="max-w-3xl mx-auto p-4 flex flex-col gap-6" onSubmit={onsubmit}>
      <h1 className="text-2xl font-semibold text-center">{type === 'create' ? 'Create a New student' : 'Update Student'}</h1>
